...
const url=process.argv[2],out=process.argv[3],btnText=process.argv[4];
...
if(btnText){ await p.evaluate(t=>{ const el=[...document.querySelectorAll('button,a')].find(b=>b.textContent.trim().includes(t)); if(el) el.click(); }, btnText); await new Promise(r=>setTimeout(r,800)); }
