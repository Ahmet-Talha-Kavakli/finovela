...
export function WatchlistCard() {
  ...
  return (
    <GlassCard hover className="h-full">
      <SectionLabel
        action={
          <Link href="/dashboard/markets" className="flex items-center gap-1 text-xs font-medium text-[var(--ais-fg-faint)] transition hover:text-[var(--ais-fg)]">
            Piyasalar
          </Link>
        }
      >
      ...
      {list.length === 0 ? (
        <p className="py-6 text-center text-sm text-[var(--ais-fg-faint)]">
          ...
          {list.map((sym) => {
            ...
            return (
              <div key={sym} className="vela-row group flex items-center gap-3 rounded-2xl px-2 py-2.5">
                ...
                <button
                  onClick={() => remove(sym)}
                  className="grid h-6 w-6 shrink-0 place-items-center rounded-full text-[var(--ais-fg-faint)] opacity-0 transition hover:bg-[var(--ais-surface-2)] hover:text-[var(--ais-fg)] group-hover:opacity-100"
